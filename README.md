...
|proxy|(Optional) A proxy if it is required for the vSensor to access the Darktrace Master instance. It should be specified in the format http://hostname:port with no authentication, or http://user:pass@hostname:port with authentication.|string|empty|no|
